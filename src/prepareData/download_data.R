# Script to download 1000 genomes rna bam files, vcf files and annotation
library(data.table)
library(Rsamtools)
...
fwrite(anno, 'Data/input_data/annotations/sample_anno_raw.txt', sep = '\t', row.names = FALSE, quote = FALSE)
...
anno_final <- unique(anno[,.(RNA_ID=`Assay Name`, 
                             WES_ID=`Source Name`,
                             SEX=`Characteristics[sex]`,
                             ORIGIN=`Characteristics[ancestry category]`, 
                             LAB=Performer, 
                             INDIVIDUAL=`Factor Value[individual]`,
...
write.table(anno_final, file.path(raw_data, "annotation.tsv"), sep="\t", row.names=FALSE, quote = FALSE)

# file mapping (ID -> filename)
anno_filemapping <- anno_final[,.(ID=RNA_ID, ASSAY="RNA_ID",
                                  FILE=file.path(raw_rna_data, paste0(RNA_ID, ".bam")))]
write.table(anno_filemapping, file.path(raw_data, "filemapping.tsv"), sep="\t", row.names=FALSE, quote = FALSE)
